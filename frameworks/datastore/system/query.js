...
SC.Query = SC.Object.extend(SC.Copyable, SC.Freezable,
  ...
  */
  ...
  //
  /**
    Takes a list of tokens (use tokenizeString; remove last token if incomplete)
    and returns a list of possible next tokens based on the grammar definition
    provided.

    Note that this method's validation is only done as far as is necessary to
    suggest possible next tokens, and should not be used to validate entire lists.

    This method assumes the format "PROPERTY OPERATOR VALUE" and will not suggest
    the valid format "VALUE OPERATOR PROPERTY". Think "myVal = 4" instead of the
    equivalent "4 = myVal". It will suggest comparing two properties though, like:
    "prop1 > prop2".

    This method assumes that boolean operators like AND, OR, et cetera operate on
    expressions rather than individual values. It does not suggest "= true" as a
    continuation for a complete expression; it will not suggest "myVal = 4 = true"
    for example.

    @param {Array} partial list of tokens
    @param {Object} grammar the grammar definition (normally queryLanguage)
    @returns {Array} list of candidate tokens
    @throws {Error} if list of tokens is invalid (e.g. CLOSE_PAREN before OPEN_PAREN)
  */
  predictNextToken: function(tokenList, grammar) {
    ...
    // ...we're following a expression-level boolean like AND, OR, NOT, et cetera.
    ...
    else throw new Error("SC.Query#predictNextToken ran into a situation that it didn't expect with token %@.".fmt(priorTokenType));


    // With the position in hand, let's figure out what's acceptable.
    ...
    // At the end, we want a primitive. (PROPERTY is okay.)
    else if (position === END) {
      for (key in grammar) {
        token = grammar[key];
        if (token.evalType === 'PRIMITIVE') ret.push(key);
      }
    }
